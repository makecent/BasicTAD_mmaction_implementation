# apart from mmaction with minor modification
...
from mmaction.models.utils.embed import PatchEmbed3D
...
class TRN(torch.nn.Module):
    ...
    def __init__(self, t_size):
        super().__init__()
        self.gamma = torch.nn.Parameter(torch.zeros(1, t_size, 1, 1), requires_grad=True)
        self.beta = torch.nn.Parameter(torch.zeros(1, t_size, 1, 1), requires_grad=True)

    def forward(self, x, out_size):
        # [N, T*H*W, C]
        residual = x
        T, H, W = out_size
        x = x.unflatten(dim=1, sizes=(T, H * W))
        # [N, T, H*W, C]
        l2 = x.norm(p=2, dim=-2, keepdim=True)
        alpha = l2 / l2.sum(dim=1, keepdim=True)
        # [N, T, 1, C]
        x = self.gamma * alpha * x + self.beta
        # [N, T, H*W, C]
        out = x.flatten(start_dim=1, end_dim=2) + residual
        # [N, T*H*W, C]
        return out
...
class MultiScaleBlock(BaseModule):
    ...
    def forward(self, x: torch.Tensor, in_size: Tuple[int]) -> tuple:
        ...
        x_norm = self.trn(self.norm2(x), out_size)
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            ...
            if i in self.downscale_indices:
                stride_q = [1, 2, 2] if not t_downscale else [2, 2, 2]
